...
def load_metrics(
    ...
    """
    Load main metric, "best" metric tracker, MetricCollection of additional metrics
    Args:
        metrics_cfg: Metric config

    Returns:
        Tuple[Metric, Metric, ModuleList]: Main metric, 'best' metric tracker, Metric Collection of additional metrics.

    """
    main_metric = hydra.utils.instantiate(metrics_cfg.main)
    if not metrics_cfg.get("valid_best"):
        raise RuntimeError(
            "Requires valid_best metric that would track best state of Main metric. \
            Usually it can be MaxMetric or MinMatric"
        )
