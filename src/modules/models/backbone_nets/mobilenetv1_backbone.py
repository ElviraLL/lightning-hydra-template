...
class MobileNet(nn.Module):
    def __init__(self, widen_factor=1.0, num_classes=1000, prelu=False, input_channel=3):
        ...
        self.fc_ori = nn.Linear(int(1024 * widen_factor), self.num_ori)
        self.fc_shape = nn.Linear(int(1024 * widen_factor), self.num_shape)
        self.fc_exp = nn.Linear(int(1024 * widen_factor), self.num_exp)
        self.fc_tex = nn.Linear(int(1024 * widen_factor), self.num_texture)
    ...
    def forward(self, x):
        ...
        ### Multi-decoder output
        x_ori = self.fc_ori(x)
        x_shp = self.fc_shape(x)
        x_exp = self.fc_exp(x)
        # x = torch.cat((x_ori, x_shp, x_exp), dim=1)

        x_tex = self.fc_tex(x)
        x = torch.cat((x_ori, x_shp, x_exp, x_tex), dim=1)
