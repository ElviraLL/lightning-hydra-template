...
from utils.params import ParamsPack # TODO: change this to hydra

param_pack = ParamsPack()

from src.modules.models.backbone_nets import mobilenetv2_backbone, resnet_backbone, ghostnet_backbone, \
    mobilenetv1_backbone
from src.modules.models.backbone_nets.pointnet_backbone import MLP_for, MLP_rev
from src.modules.losses import ParamLoss, WingLoss

from src.modules.models.backbone_nets.ResNeSt import resnest50
...
class SynergyNet(nn.Module):
    ...
    def reconstruct_vertex_62(self, param, whitening=True, dense=False, transform=True, lmk_pts=68):
        ...
        if dense:
            ...
            vertex = p @ (self.u + self.w_shp @ alpha_shp + self.w_exp @ alpha_exp).contiguous().view(-1, 53215,
                                                                                                      3).transpose(1,
                                                                                                                   2) + offset
            ...
            """For 68 pts"""
            vertex = p @ (self.u_base + self.w_shp_base @ alpha_shp + self.w_exp_base @ alpha_exp).contiguous().view(-1,
                                                                                                                     lmk_pts,
                                                                                                                     3).transpose(
                1, 2) + offset
